...
class Residual(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        feature = self.backbone(input)  # B*512*15*20
        # print(feature.shape)
        up1 = F.interpolate(feature, scale_factor=2, mode='bilinear', align_corners=True)  # B*512*30*40
        # print(up1.shape)
        up1 = self.up_layer1(up1)  # B*256*30*40
        # print(up1.shape)
        up1 = F.interpolate(up1, scale_factor=2, mode='bilinear', align_corners=True)  # B*256*60*80
        # print(up1.shape)
        up1 = self.up_layer2(up1)  # B*128*60*80
        # print(up1.shape)

        cat1 = torch.cat([up1, F.interpolate(input, size=up1.shape[2:])], 1)  # B*132*60*80
        # print(cat1.shape)

        up2 = F.interpolate(cat1, scale_factor=2, mode='bilinear', align_corners=True)  # B*132*120*160
        # print(up2.shape)
        up2 = self.up_layer3(up2)  # B*64*120*160
        # print(up2.shape)
        up2 = F.interpolate(up2, scale_factor=2, mode='bilinear', align_corners=True)  # B*64*240,320
        # print(up2.shape)
        up2 = self.up_layer4(up2)  # B*32*240*320
        # print(up2.shape)

        cat2 = torch.cat([up2, F.interpolate(input, size=up2.shape[2:])], 1)  # B*36*240*320
        # print(cat2.shape)

        up = F.interpolate(cat2, scale_factor=2, mode='bilinear', align_corners=True)  # B*32*480*640
        # print(up.shape)
        up = self.up_layer5(up)  # B*16*480*640
        # print(up.shape)
        # up = F.interpolate(up, size=(385, 385), mode='bilinear', align_corners=True)  # B*16*385*385
        prediction = self.up_layer6(up)  # B*1*480*640
...
class Resample(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        # print(up.shape)
        # up = F.interpolate(up, size=(385, 385), mode='bilinear', align_corners=True)  # B*16*385*385
        prediction = self.up_layer6(up)  # B*2*480*640
        # print(prediction.shape)

        # scale need to specify
        prediction = self.predict(prediction)  # B*2*480*640
        # print(prediction.shape)
...
class Refine_module2(torch.nn.Module):

    def __init__(self, load_pretrained=True, pretrained_resnet18_path=None):
        super(Refine_module2, self).__init__()
    ...
    def forward(self, depth_tensor, img_tensor, enlarge=True):
        ...
        # depth_mask=get_mask(enlarged_depth)
        ...
        resampled_depth = self.sample(resample_grid * (torch.stack([mask.squeeze(1), mask.squeeze(1)], 3)),
                                      compensate_res_depth)
